...
//        if(Input.GetKeyDown(KeyCode.Keypad0)){ATTACK("blizzard", 1, 5);}
    ...
    void Select(int charSelect){ //temp for manual input
        Debug.Log(charSelect.ToString());

        if (InputCounter[0] == 0) { //caster
            InputCounter[1] = charSelect;
            InputCounter[0]++;
        }
        else if (InputCounter[0] == 1) { //spell
        ...
        else if (InputCounter[0] == 2) { //target
    ...
    string numString(int Num){ //temp for manual input
        switch (Num) {
            case 1:
                return "heal";
            case 2:
...
public class Attacks { //Attack Dictionary 
